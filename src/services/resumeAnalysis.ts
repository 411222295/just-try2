...
interface AnalysisInput extends ResumeData {
  motivation?: string;
  industryAndJob?: string;
  clubExperience?: string;
}

export const analyzeResume = async (inputData: AnalysisInput): Promise<string[]> => {
  // 這裡應該調用 OpenAI API 來分析履歷
  // 目前使用模擬數據
  const skills = inputData.skills;
  ...
  if (inputData.motivation) {
    tags.push(`求職動機: ${inputData.motivation}`);
  }
  if (inputData.industryAndJob) {
    tags.push(`尋找職位: ${inputData.industryAndJob}`);
  }
  if (inputData.clubExperience) {
    tags.push(`社團經驗: ${inputData.clubExperience}`);
  }
